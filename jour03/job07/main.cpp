...
int main() {
    ...
    while (true) {
        ...
        Character* playerPtr = world.getPlayer();
        Character* enemyPtr = world.getEnemy();

        if (playerPtr && enemyPtr) {
            std::cout << playerPtr->getName() << " has " << playerPtr->getHealthPoints() << " HP remaining.\n";
            std::cout << enemyPtr->getName() << " has " << enemyPtr->getHealthPoints() << " HP remaining.\n";

            if (!playerPtr->isAlive()) {
                std::cout << playerPtr->getName() << " is dead. Game Over.\n";
                break;
            }

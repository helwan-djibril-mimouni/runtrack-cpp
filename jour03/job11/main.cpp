...
public:
    ...
    virtual bool collidesWith(const GameObject& other) const {
        return (std::abs(getX() - other.getX()) < 1.0 && std::abs(getY() - other.getY()) < 1.0);
    }
        ...
        if (isAlive() && target) {
            ...
            if (dist > 1) { // Si l'ennemi n'est pas encore à portée
                ...
                if (!target->collidesWith(*this)) {
                    ...
                    std::cout << getName() << " cannot move due to collision.\n";
                }
...
public:
    ...
    bool checkCollision(const GameObject* movingObject) const {
        for (const auto& obj : objects) {
            if (obj.get() != movingObject && movingObject->collidesWith(*obj)) {
                return true;
            }
        }
        return false;

...
public:
    ...
    static std::unordered_map<std::string, std::string> lieuxImportants;
    ...
    void ajouterTemps(int t);
    void afficherTemps();
    void ajouterPeche(std::string lieu);
    void retirerPeche(std::string lieu);
    void afficherPeche();
    ...
    void allerLieuImportant(std::string lieu) {
        auto it = lieuxImportants.find(lieu);
        if (it != lieuxImportants.end()) {
            std::cout << get_nom() << " se rend au lieu important : " << lieu << "." << std::endl;
            std::cout << "Il peut trouver du poisson au lieu de peche: " << it->second << "." << std::endl;

            if (std::rand() % lieu.size() == 0) {
                std::cout << "Bonne nouvelle ! Du poisson a ete recupere au lieu de peche" << std::endl;
            } else {
                std::cout << "Pas de poisson trouve cette fois ci" << std::endl;
            }
        } else {
            std::cout << "Le lieu importnant '" << lieu << "' n'est pas enregistre" << std::endl;
        }
    };

    static void ajouterLieuImportant(std::string lieu, std::string lieuPeche) {
        lieuxImportants[lieu] = lieuPeche;
    };

    static void supprimerLieuImportant(const std::string& lieu) {
        lieuxImportants.erase(lieu);
    };

    static void afficherLieuxImportants() {
        std::cout << "Lieux importants et lieux de pêche :" << std::endl;
        for (const auto& paire : lieuxImportants) {
            std::cout << "Lieu importants : " << paire.first << " | Lieu de pêche: " << paire.second << std::endl;
        }
    };
};
...
void Pingouin::ajouterTemps(int t) {
    temps.insert(t);
}

void Pingouin::afficherTemps() {
    std::cout << "Temps realise par " << get_nom() << " en 100m glisse :" << std::endl;
    for (const auto& t : temps) {
        std::cout << "- " << t << " secondes" << std::endl;
    }
}

void Pingouin::ajouterPeche(std::string lieu) {
    lieuxPeche.insert(lieu);
}

void Pingouin::retirerPeche(std::string lieu) {
    lieuxPeche.erase(lieu);
}

void Pingouin::afficherPeche() {
    std::cout << "Lieux de peche preferes de " << get_nom() << " :" << std::endl;
...
int main()
{
    ...
    pingouin->ajouterTemps(10);
    pingouin->ajouterTemps(11);
    pingouin->ajouterTemps(9);
    pingoudeux->ajouterTemps(12);
    pingouin->afficherTemps();
    pingoudeux->afficherTemps();
    pingouin->ajouterPeche("glacier");
    pingouin->ajouterPeche("lac");
    pingouin->ajouterPeche("mer");
    pingoudeux->ajouterPeche("mer");
    pingoutrois->ajouterPeche("lac");
    pingouquatre->ajouterPeche("montagne");
    pingouin->afficherPeche();
    pingoudeux->afficherPeche();
    pingoutrois->afficherPeche();
    pingouquatre->afficherPeche();
}